#!/usr/bin/env bash
# Single-language conformance driver for good-sender/go-sdk.
#
# Usage:
...
build_sdk() { :; }   # go modules don't pre-build

install_runner() {
  (cd "$SCRIPT_DIR" && go mod tidy >/dev/null 2>&1) || true
}
...
case "$MODE" in
  mock)
    ...
    build_sdk
    install_runner
    (cd "$SCRIPT_DIR" && go run runner.go)
    ;;
  dev)
    ...
    build_sdk
    install_runner
    (cd "$SCRIPT_DIR" && go run -tags conformance .)
    ;;
